...
module tb_bf16_mul;
    ...
    localparam int N = 89;  // must match number of lines in vectors/
    ...
    initial begin
        $readmemh("vectors/mul_in_a.txt",   mem_a);
        $readmemh("vectors/mul_in_b.txt",   mem_b);
        $readmemh("vectors/mul_golden.txt", mem_g);

        pass_cnt = 0;
        fail_cnt = 0;

        $display("================================================================");
        $display("  BF16 MUL TESTBENCH  (%0d cases)", N);
        $display("================================================================");
