...
\begin{enumerate}
    ...
    The error decreases monotonically and falls below the $2^{-9}$ criterion at $w = 9$, after which further increase yields negligible improvement.
    Therefore, the fractional wordlength of the $X$/$Y$ datapath is set to $w = 9$ bits.
    ...
    Combined with the sign bit, the complete word format is (1 sign $+$ 1 integer $+$ 9 fractional) $= 11$ bits.

    \begin{figure}[htbp]
        \centering
        \includegraphics[width=1.0\textwidth]{../diagram/Q2/phase_error_vs_wordlength.png}
        \caption{Average absolute phase error versus fractional wordlength $w$ of $X(i)$/$Y(i)$ ($N = 30$, floating-point $\theta$)}
    ...
    Figure~\ref{fig:error_vs_S} shows the average absolute phase error versus the number of micro-rotations $S$, with the $X$/$Y$ fractional wordlength fixed at 9 bits.
    The error decreases as $S$ increases and first falls below the $2^{-9}$ criterion at $S = 10$.
    Therefore, the number of micro-rotations is set to $S = 10$.

    Figure~\ref{fig:error_vs_angle_wordlength} shows the average absolute phase error versus the fractional wordlength of the quantized elementary angles, with $S = 10$ micro-rotations and $X$/$Y$ fractional wordlength fixed at 9 bits.
    The error decreases with increasing wordlength, meets the $2^{-9}$ criterion at 10 bits, and saturates beyond that point because the dominant error source shifts to the $X$/$Y$ quantization.
    ...
    \begin{figure}[htbp]
        ...
        \caption{Average absolute phase error versus number of micro-rotations $S$ (fractional wordlength of $X$/$Y$ = 9 bits, floating-point elementary angles)}
    ...
    \begin{figure}[htbp]
        ...
        \caption{Average absolute phase error versus fractional wordlength of quantized elementary angles ($S = 10$, fractional wordlength of $X$/$Y$ = 9 bits)}
    ...
    \input{elementary_angles_table.tex}
    ...
    \begin{figure}[htbp]
        ...
        \caption{Magnitude function error versus number of micro-rotations $N$}
    ...
    \begin{figure}[htbp]
        \centering
        % \includegraphics[width=1.0\textwidth]{../diagram/Q5/csd_error_vs_wordlength.png}
        \caption{CSD scaling factor error versus fractional wordlength}
        \label{fig:csd_error_vs_wordlength}
    \end{figure}

    \begin{figure}[htbp]
        \centering
        % \includegraphics[width=0.75\textwidth]{../diagram/Q5/shift_add_block.png}
        \caption{Shift-and-add block diagram for the CSD scaling factor}
        \label{fig:shift_add_block}
    \end{figure}
    ...
    \begin{figure}[htbp]
        \centering
        % \includegraphics[width=0.75\textwidth]{../diagram/Q6/initial_stage.png}
